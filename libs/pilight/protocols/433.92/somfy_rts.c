...
static void createMessage(char *message, int address, int command, int rollingcode, int rollingkey, int repeats) {
	char str [9];
    int x = snprintf(message, 255, "{\"address\":%d,", address);
    if(command==1) {
        x += snprintf(&message[x], 255-x, "\"state\":\"my\"");
    } else if (command==2) {
        x += snprintf(&message[x], 255-x, "\"state\":\"up\"");
    } else if (command==3) {
        x += snprintf(&message[x], 255-x, "\"state\":\"my+up\"");
    } else if (command==4) {
        x += snprintf(&message[x], 255-x, "\"state\":\"down\"");
    } else if (command==5) {
        x += snprintf(&message[x], 255-x, "\"state\":\"my+down\"");
    } else if (command==6) {
        x += snprintf(&message[x], 255-x, "\"state\":\"up+down\"");
    } else if (command==8) {
        x += snprintf(&message[x], 255-x, "\"state\":\"prog\"");
    } else if (command==9) {
        x += snprintf(&message[x], 255-x, "\"state\":\"sun+flag\"");
    } else if (command==10) {
        x += snprintf(&message[x], 255-x, "\"state\":\"flag\"");
    } else {
		sprintf(str, "%d", command);
        x += snprintf(&message[x], 255-x, "\"state\":%s,", str);
	}
	if (rollingcode != -1) {
		x += snprintf(&message[x], 255-x, "\"rollingcode\":%d,", rollingcode);
	}
	if (rollingkey != -1) {
		x += snprintf(&message[x], 255-x, "\"rollingkey\":%d,", rollingkey);
	}
...
//			break;
	...
	if (protocol_sync > 97) {
		...
		if ( ( key_left == 10 && 0 == cksum)) {
			createMessage(message, address, command, rollingcode, rollingkey, repeats);
		}
	}
}
