/*
	Copyright (C) 2014 - 2016 CurlyMo & wo_rasp
...
static void createMessage(char *message, int device_id, int id, int unit, int battery, double temp, double humidity, int uv, int wind_dir, int wind_speed, int wind_avg, int rain, int rain_total, int pressure) {

	int x = snprintf(message, 255, "{\"device_id\":%d",device_id);
	...
	if (!((temp+27400.0) < EPSILON))
		x += snprintf(&message[x], 255- x, ",\"temperature\":%.2f",temp/100.0);
	if (!((humidity+1.0) < EPSILON))
		x += snprintf(&message[x], 255-x, ",\"humidity\":%1f",humidity);
	if (uv != -1)
		x += snprintf(&message[x], 255-x, ",\"uv\":%d",uv);
	if (wind_dir != -1)
		x += snprintf(&message[x], 255-x, ",\"winddir\":%d",wind_dir);
	if (wind_speed != -1)
		x += snprintf(&message[x], 255-x, ",\"windgust\":%d",wind_speed);
	if (wind_avg != -1)
		x += snprintf(&message[x], 255-x, ",\"windavg\":%d",wind_avg);
	if (rain != -1)
		x += snprintf(&message[x], 255-x, ",\"rain\":%d",rain);
	if (rain_total != -1)
		x += snprintf(&message[x], 255-x, ",\"raintotal\":%d",rain_total);
	if (pressure != -1)
		x += snprintf(&message[x], 255-x, ",\"pressure\":%d",pressure);
...
// and remove all inverted bits
	...
	// ad checksum check later on
		...
		if (b_unknown && (eBin == pBin || eBin == 0)) {
			createMessage(message, device_id, id, unit, battery, temp, humidity, uv, wind_dir, wind_speed, wind_avg, rain, rain_total, pressure);
		}
